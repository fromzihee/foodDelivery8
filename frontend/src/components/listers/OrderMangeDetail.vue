<template>
    <v-card outlined>
        <v-card-title>
            OrderMange # {{item._links.self.href.split("/")[item._links.self.href.split("/").length - 1]}}
        </v-card-title>
        ...
        <v-card-text>
            ...
            <div>
                <String label="Sts" v-model="item.sts" :editMode="editMode" @change="change" />
            </div>
            <div>
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            var temp = await axios.get(axios.fixUrl('/orderManges/' + params.id))
            if(temp.data) {
                me.item = temp.data
            }
        },
